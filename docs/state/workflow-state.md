...
**Active Phase:** Spec 002 scheduler/practice (Plan 002c) — Waves 1–4 COMPLETE; Wave 5 (verification + adversarial PR/QA gate + PR) is next. Operational tracking now lives in **Beads** (`bd`, prefix `rw`); epic `rw-1gz.1` (Scheduler & practice).  
...
**Current Gate:** Waves 1–4 COMPLETE on branch `plan/002c-scheduler-practice`. Wave 1: content loader + review-rule helper. Wave 2: `api/src/scheduler/planner.ts` (buildPracticePlan, injected-content seam) + mastery upsert in `api/src/routes/practice.ts` (single D1 batch); Wave 2 adversarial Round 1 BLOCK resolved (BLOCK 1–3 fixed, IMPORTANT 4 documented, IMPORTANT 5 deferred, IMPORTANT 6 pushed back). Wave 3: route integration (grade-aware start plans, full review history retained). Wave 4 (verify-only, `rw-1gz.1.1` CLOSED): play.test.tsx uses telemetry's `completePractice` mock + contract, asserts call + navigation to `/play/student1/done`. Full post-rebase gate green: tsc clean, **api 46 tests**, **app 13 tests**, app build clean.  
**Blockers:** none. Open Beads: `rw-1gz.1.2` (Wave 5, ready), `rw-1gz.1.3` (deferred — surface 1st-grade empty-plan terminal reason at route/HTTP layer, IMPORTANT 5), `rw-1gz.4` (P3 — assert completion-failure best-effort in play.test.tsx, discovered in Wave 4).  
**Next Action:** run Wave 5 — full verification gate (`pnpm typecheck && pnpm test && pnpm content:validate`), adversarial PR/QA review (Task 15 reviewer-availability guard), then force-push the rebased branch and open the scheduler PR to main.  
**Active Snapshot Pointer:** branch `plan/002c-scheduler-practice` rebased onto main (20 commits ahead, 0 behind); Wave 4 closed in Beads; tree clean except this checkpoint edit. Local branch history rewritten by rebase — origin needs a force-push.
